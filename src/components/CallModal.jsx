...
export default function CallModal({ socket, myUsername, partnerName, chatId }) {
  ...
  const loggedRef = useRef(false); // guard against double-logging the same call
  ...
  function createPeerConnection() {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) send("ice_candidate", { payload: { candidate: e.candidate } });
    };
  ...
  async function startCall(type) {
    ...
    try {
      ...
      send("call_offer", { call_type: type, payload: { sdp: offer.sdp, type: offer.type } });
    } catch (err) {
  ...
  async function acceptCall() {
    ...
    try {
      ...
      await pc.setRemoteDescription(new RTCSessionDescription(incomingOffer.payload));
      await flushPendingCandidates();

      const answer = await pc.createAnswer();
      await pc.setLocalDescription(answer);
      send("call_answer", { payload: { sdp: answer.sdp, type: answer.type } });
      setCallState("connected");
      didConnectRef.current = true;
      connectedAtRef.current = Date.now();
    } catch (err) {
  ...
  // Ends the call, and - only if we were the caller - logs it so it shows
  // up as a call bubble in the chat (missed / declined / completed w/ duration)
  function finishCall(status) {
    if (isCallerRef.current && !loggedRef.current && chatId) {
      loggedRef.current = true;
      const duration =
        status === "completed" && connectedAtRef.current
          ? Math.round((Date.now() - connectedAtRef.current) / 1000)
          : null;
      logCall(chatId, { call_type: callType, call_status: status, duration_seconds: duration }).catch(() => {});
    }
    cleanup();
    setCallState("idle");
  }
  ...
  if (callState === "idle") {
    return (
      <div style={styles.callButtons}>
        <button onClick={() => startCall("audio")} style={styles.callIconBtn} title="Voice call">📞</button>
        <button onClick={() => startCall("video")} style={styles.callIconBtn} title="Video call">🎥</button>
      </div>
    );
  }
...
const styles = {
  callButtons: { display: "flex", gap: 4 },
  callIconBtn: { border: "none", background: "none", fontSize: 20, cursor: "pointer", padding: 4 },
  overlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 },
  ...
  errorBanner: { position: "absolute", top: 20, background: "#ffe5e9", color: "#e41e3f", padding: "8px 16px", borderRadius: 8, fontSize: 13 },
};